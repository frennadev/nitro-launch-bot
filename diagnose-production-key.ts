...
async function diagnoseProductionKey() {
  console.log(
    "🔍 EMERGENCY: Diagnosing Production Encryption Key Mismatch...\n"
  );
  ...
  try {
    ...
    if (recentErrors.length > 0) {
      console.log(
        `⚠️ Found ${recentErrors.length} wallets marked as ERROR in the last 10 minutes!`
      );
      ...
      for (let i = 0; i < Math.min(3, recentErrors.length); i++) {
        ...
        try {
          ...
          if (keypair.publicKey.toString() === wallet.publicKey) {
            console.log(
              `   ✅ ERROR wallet decrypts fine with our key! (False error)`
            );
          } else {
          ...
          console.log(
            `   🔍 This suggests production is using a DIFFERENT encryption key!`
          );
        }
      }
    } else {
    ...
    const availableWallets = await collection
    ...
    for (const wallet of availableWallets) {
      try {
        ...
        console.log(
          `   ❌ Available wallet ${wallet.publicKey.slice(0, 8)}... failed: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }

    console.log(
      `   ✅ ${workingCount}/${availableWallets.length} available wallets work with our key`
    );
    ...
    if (recentErrors.length > 0) {
      ...
      console.log(
        "   1. Check what ENCRYPTION_SECRET is set in production environment"
      );
      console.log("   2. Either update production to use the correct key");
      console.log(
        "   3. Or update our scripts to use production's current key"
      );
