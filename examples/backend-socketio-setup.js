/**
 * Complete Backend Setup Example for CTO Progress Tracking
...
// Import your CTO functions
const {
  enqueueCTOOperation,
  getCTOJobStatus,
} = require("./src/jobs/cto-external");
...
const activeConnections = new Map(); // userId -> socketId mapping
...
app.post("/api/cto/job/:jobId/cancel", async (req, res) => {
  try {
    ...
    const { cancelCTOJob } = require("./src/jobs/cto-external");
 ...
 */
app.get("/api/cto/health", (req, res) => {
  res.json({
    success: true,
    status: "healthy",
...
if (process.env.NODE_ENV === "development") {
  app.get("/api/cto/debug/connections", (req, res) => {
    res.json({
      success: true,
      data: {
...
server.listen(PORT, () => {
  ...
  console.log(`📡 API endpoints available at http://localhost:${PORT}/api/cto`);

  // Start your CTO workers here
  console.log("🔧 Starting CTO workers...");
  require("./src/jobs/index"); // This should start your workers
});
