...
async function findWalletInfo() {
  const client = new MongoClient(process.env.MONGODB_URI || 'mongodb://localhost:27017/nitro-launch-bot');
  ...
  try {
    ...
    const user = await db.collection('users').findOne({ 
      _id: require('mongodb').ObjectId.createFromHexString('68492054bc12916bc8cedcb3')
    });
    ...
    if (user) {
      ...
      if (user.fundingWallet) {
        ...
        if (fundingWallet) {
          console.log(`   - Funding Wallet Public Key: ${fundingWallet.publicKey}`);
        }
      }
    } else {
