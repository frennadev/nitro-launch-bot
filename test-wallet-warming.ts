...
async function testWalletWarming() {
  try {
    ...
    console.log(
      `✅ Found user: ${user.userName || user.firstName || "Unknown"}`
    );
    ...
    const populatedFundingWallet = user.fundingWallet as unknown as {
      publicKey: string;
    };
    console.log(`✅ Found funding wallet: ${populatedFundingWallet.publicKey}`);

    // Check if the wallet to warm exists
    const walletToWarmDoc = await WalletModel.findOne({
      publicKey: walletToWarm,
    });
    if (!walletToWarmDoc) {
    ...
    console.log(
      "📝 The wallet warming job has been queued and should start processing."
    );
    console.log(
      "🔄 You can monitor the progress through the Socket.IO events or check the wallet document in the database."
    );
