...
async function sellWithYourWallet() {
  try {
    ...
    if (PRIVATE_KEY.length !== 64) {
      console.log('❌ Please provide your private key in the PRIVATE_KEY array');
      console.log('💡 Export your private key from your wallet as a byte array');
    ...
    if (solBalance < 0.005 * LAMPORTS_PER_SOL) {
      console.log('⚠️  WARNING: Low SOL balance. You need at least 0.005 SOL for transaction fees');
      return;
    }
    ...
    try {
      ...
      console.log('❌ No token account found - you have no tokens of this mint');
      return;
    }
    ...
    const tokensToSell = tokenBalance > BigInt(10000) 
    ...
    console.log(`📤 Selling: ${tokensToSell.toString()} tokens (${((Number(tokensToSell) / Number(tokenBalance)) * 100).toFixed(2)}% of balance)`);
    ...
    console.log(`\n📊 Updated Balances:`);
    console.log(`💰 SOL: ${(newSolBalance / LAMPORTS_PER_SOL).toFixed(4)} SOL (${newSolBalance > solBalance ? '+' : ''}${((newSolBalance - solBalance) / LAMPORTS_PER_SOL).toFixed(4)})`);
    console.log(`🪙 Tokens: ${newTokenBalance.value.amount} (${BigInt(newTokenBalance.value.amount) < tokenBalance ? '-' : '+'}${(tokenBalance - BigInt(newTokenBalance.value.amount)).toString()})`);
