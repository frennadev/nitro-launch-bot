...
async function fixAllWallets() {
  ...
  try {
    ...
    const totalWallets = statusCounts.reduce((sum, status) => sum + status.count, 0);
    ...
    const nonAvailableWallets = await collection.find({ 
      status: { $ne: "available" }
    }).toArray();
    
    console.log(`Found ${nonAvailableWallets.length} wallets to check and potentially fix`);
    ...
    for (let i = 0; i < nonAvailableWallets.length; i += batchSize) {
      ...
      for (const wallet of batch) {
        ...
        if (wallet.status === "in_use") {
          ...
          const now = new Date();
          const hoursSinceLastUse = lastUsed ? (now.getTime() - lastUsed.getTime()) / (1000 * 60 * 60) : null;
          
          if (hoursSinceLastUse && hoursSinceLastUse > 1) {
            // If not used in the last hour, it's probably stuck as "in_use"
            console.log(`   🔄 ${wallet.publicKey.slice(0, 8)}... - Stuck as in_use (${hoursSinceLastUse.toFixed(1)}h ago), checking...`);
          } else {
            console.log(`   ⏳ ${wallet.publicKey.slice(0, 8)}... - Recently in use, skipping`);
            inUseCount++;
            continue;
          }
        }
        ...
        try {
          ...
          if (keypair.publicKey.toString() === wallet.publicKey) {
            ...
            console.log(`   ❌ ${wallet.publicKey.slice(0, 8)}... - Public key mismatch`);
            stillBrokenCount++;
            stillBrokenWallets.push(wallet.publicKey);
          }
          
        } catch (error) {
          console.log(`   ❌ ${wallet.publicKey.slice(0, 8)}... - Decryption failed: ${error instanceof Error ? error.message : String(error)}`);
          stillBrokenCount++;
          stillBrokenWallets.push(wallet.publicKey);
        }
      }
    ...
    console.log(`   📈 Total wallets processed: ${fixedCount + inUseCount + stillBrokenCount}`);
    ...
    console.log(`   💪 This supports mixing to ${maxConcurrentRoutes} wallets simultaneously with full privacy!`);
