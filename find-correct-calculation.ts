...
async function findCorrectCalculation() {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      console.log(
        `   Virtual Base: ${Number(poolData.virtualBase).toLocaleString()}`
      );
      console.log(
        `   Virtual Quote: ${Number(poolData.virtualQuote).toLocaleString()}`
      );
      console.log(
        `   Real Base: ${Number(poolData.realBase).toLocaleString()}`
      );
      console.log(
        `   Real Quote: ${Number(poolData.realQuote).toLocaleString()}`
      );
      ...
      console.log(
        `   Virtual SOL Price: ${virtualPrice.toExponential(4)} SOL/token`
      );
      ...
      const methods = [
        ...
        },
        {
          ...
          price:
            (virtualPrice * virtualQuote +
              (realBase > 0 ? realQuote / realBase : 0) * realQuote) /
            (virtualQuote + realQuote),
          supply: totalSupply,
      ...
      methods.forEach((method, index) => {
        ...
        console.log(
          `   Difference: $${diff.toLocaleString()} (${((diff / target) * 100).toFixed(1)}% off)`
        );
      ...
      console.log(
        `   Accuracy: ${(100 - (closest.diff / target) * 100).toFixed(1)}%`
      );
      ...
      console.log(
        `   Multiplier needed: ${(targetPriceSOL / virtualPrice).toFixed(3)}x`
      );
      ...
      const experiments = [
        ...
        },
        {
          ...
          price: Math.sqrt(
            virtualPrice * (realBase > 0 ? realQuote / realBase : 0)
          ),
      ...
      experiments.forEach((exp, i) => {
        ...
        console.log(
          `${i + 1}. ${exp.name}: $${marketCap.toLocaleString()} (${exp.desc})`
        );
