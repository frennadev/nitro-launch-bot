...
async function findProductionKey() {
  ...
  try {
    ...
    const testManager = new MongoWalletManager(mongoUri, databaseName, possibleKeys[0]);
    ...
    const recentErrorWallets = await collection.find({ 
      status: "error",
      createdAt: { $gte: new Date(Date.now() - 60 * 60 * 1000) } // Last hour
    }).limit(5).toArray();
    
    console.log(`Found ${recentErrorWallets.length} recent ERROR wallets to test with different keys`);
    
    if (recentErrorWallets.length === 0) {
      console.log("❌ No recent ERROR wallets found - production might not be running");
    ...
    for (let i = 0; i < possibleKeys.length; i++) {
      ...
      for (const wallet of recentErrorWallets.slice(0, 3)) { // Test first 3
        try {
      ...
      console.log(`   ✅ Success: ${successCount}/${recentErrorWallets.slice(0, 3).length} wallets`);
      console.log(`   ❌ Failed: ${failCount}/${recentErrorWallets.slice(0, 3).length} wallets`);
      ...
      if (successCount === recentErrorWallets.slice(0, 3).length) {
        ...
        console.log(`✅ Confirmed: ${confirmSuccess}/${recentErrorWallets.length} recent ERROR wallets work with this key`);
      }
    ...
    console.log("1. Check what ENCRYPTION_SECRET is set in your production environment");
