...
async function investigateBonkTokens() {
  ...
  for (const mintAddress of TOKENS_TO_INVESTIGATE) {
    ...
    try {
      ...
      const pumpfunAccount = await connection.getAccountInfo(pumpfunBondingCurve);
      if (pumpfunAccount) {
        console.log(`   ✅ PumpFun bonding curve found: ${pumpfunBondingCurve.toBase58()}`);
      ...
      try {
        ...
        if (largestAccounts.value.length > 0) {
          console.log(`   💰 Largest holder: ${largestAccounts.value[0].amount} tokens`);
          console.log(`   👥 Total accounts with balance: ${largestAccounts.value.filter(acc => acc.amount !== '0').length}`);
        }
      } catch (error) {
  ...
  console.log('\n📋 ANALYSIS:');
  console.log('These tokens appear to be standard SPL tokens, not bonding curve tokens.');
