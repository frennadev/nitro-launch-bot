...
async function checkSingleToken() {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      console.log(
        `🎯 Status: ${data.isComplete ? "🎓 Graduated" : "🔄 On Curve"}`
      );
      console.log(`🏊 Migrated: ${data.isMigrated ? "✅ Yes" : "❌ No"}`);
      console.log(
        `👤 Creator: ${data.creator.slice(0, 8)}...${data.creator.slice(-8)}`
      );
      ...
      console.log(
        `🔄 Circulating: ${data.circulatingSupply.toLocaleString()} tokens`
      );
      console.log(`🏊 SOL Reserves: ${data.solReserves.toFixed(4)} SOL`);
      console.log(
        `🪙 Token Reserves: ${data.tokenReserves.toLocaleString()} tokens`
      );

      if (
        "graduationProgress" in data &&
        data.graduationProgress !== undefined
      ) {
        console.log(
          `📈 Graduation Progress: ${data.graduationProgress.toFixed(2)}%`
        );
      }
    } else {
