...
/**
 * PumpFun Token Creation + Dev Buy Test Script
 * 
 * Usage: bun run test-pumpfun-create-devbuy.ts <private_key> <name> <symbol> <dev_buy_amount>
 * 
 * Example: bun run test-pumpfun-create-devbuy.ts "your_private_key_here" "TestToken" "TEST" 0.1
 */
...
import { uploadJsonToPinata } from "./src/backend/utils";
...
async function main() {
  ...
  if (args.length !== 4) {
    console.log("❌ Usage: bun run test-pumpfun-create-devbuy.ts <private_key> <name> <symbol> <dev_buy_amount>");
    console.log("📝 Example: bun run test-pumpfun-create-devbuy.ts \"your_private_key_here\" \"TestToken\" \"TEST\" 0.1");
    process.exit(1);
  }
  ...
  console.log("🚀 PumpFun Token Creation + Dev Buy Test");
  ...
  try {
    ...
    const metadata = {
      name: tokenName,
      symbol: symbol,
      description: `${tokenName} (${symbol}) - Created with PumpFun Test Script`,
      image: "https://via.placeholder.com/512x512.png?text=" + encodeURIComponent(symbol),
      external_url: "",
      attributes: [],
      properties: {
        files: [
          {
            uri: "https://via.placeholder.com/512x512.png?text=" + encodeURIComponent(symbol),
            type: "image/png"
          }
        ],
        category: "image"
    ...
    const metadataHash = await uploadJsonToPinata(metadata, `${tokenName}-${symbol}-metadata`);
    ...
    // 2. Dev buy instructions (if dev buy > 0)
    ...
    if (devBuyAmount > 0) {
      ...
      console.log("✅ Dev ATA creation instruction added");
