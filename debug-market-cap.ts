...
async function debugMarketCapCalculation() {
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      console.log(`🔢 Pool Status: ${poolData.status} (0=funding, 1=waiting migration, 2=migrated)`);
      ...
      console.log(`🔵 Adjusted Virtual Base: ${adjustedVirtualBase.toLocaleString()}`);
      console.log(`🔵 Adjusted Virtual Quote: ${adjustedVirtualQuote.toFixed(4)} SOL`);
      console.log(`🟢 Adjusted Real Base: ${adjustedRealBase.toLocaleString()}`);
      console.log(`🟢 Adjusted Real Quote: ${adjustedRealQuote.toFixed(4)} SOL`);
      ...
      console.log(`💰 Method 1 (Virtual): ${adjustedVirtualQuote.toFixed(4)} / ${adjustedVirtualBase.toLocaleString()} = ${priceMethod1.toExponential(4)} SOL per token`);
      
      // Method 2: Real reserves
      const priceMethod2 = adjustedRealBase > 0 ? adjustedRealQuote / adjustedRealBase : 0;
      console.log(`💰 Method 2 (Real): ${adjustedRealQuote.toFixed(4)} / ${adjustedRealBase.toLocaleString()} = ${priceMethod2.toExponential(4)} SOL per token`);
      ...
      console.log(`💰 Method 3 (Combined): ${totalQuote.toFixed(4)} / ${totalBase.toLocaleString()} = ${priceMethod3.toExponential(4)} SOL per token`);
      ...
      console.log(`📊 Current Calculation: $${data.marketCap.toLocaleString()}`);
      ...
      else if (minDiff === diff3) console.log('🏆 Method 3 (Combined) is closest!');
      else console.log('🏆 Current method is closest!');
      
      // Check circulating supply approach
      console.log('\n🔄 CIRCULATING SUPPLY APPROACH:');
      const circulatingMarketCap = data.circulatingSupply * priceMethod1 * solPrice;
      console.log(`📊 Circulating Supply: ${data.circulatingSupply.toLocaleString()}`);
      console.log(`📊 Circulating Market Cap: $${circulatingMarketCap.toLocaleString()}`);
