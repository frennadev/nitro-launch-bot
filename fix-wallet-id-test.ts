...
async function findAndTestWalletWarming() {
  try {
    ...
    if (!walletDoc) {
      ...
      console.log(
        "✅ Created wallet document with ID:",
        savedWallet._id.toString()
      );
      ...
      console.log(
        "✅ Found existing wallet document with ID:",
        walletDoc._id.toString()
      );
    ...
    console.log(
      "📝 The wallet warming job has been queued with the correct MongoDB document ID."
    );
    console.log(
      "🔄 Monitor the worker logs to see the warming process in action."
    );
  } catch (error) {
