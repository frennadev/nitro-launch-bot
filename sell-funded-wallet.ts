...
const walletPrivateKey = '4ERTpLTjjJ9vP2MES7hs1HZw3VB9z5kvzP28sodwaPk4uY4Hr6g1Qqo48tM9FaZiq8Y8wcUKrYEECNA4euqD5J7Q';
  ...
  try {
    ...
    try {
      ...
      if (balance.value.decimals) {
        const readableBalance = Number(tokenBalance) / Math.pow(10, balance.value.decimals);
        console.log(`📊 Readable Balance: ${readableBalance.toLocaleString()} tokens`);
      }
    } catch (error) {
    ...
    const tokensToSell = tokenBalance > BigInt(100000) 
    ...
    console.log(`📊 Percentage: ${((Number(tokensToSell) / Number(tokenBalance)) * 100).toFixed(2)}% of balance`);
    ...
    sellIx.keys.forEach((key, index) => {
      ...
      console.log(`  ${index.toString().padStart(2)}: ${name.padEnd(20)} - ${key.pubkey.toString().substring(0, 8)}...`);
    });
    ...
    console.log(`\n📊 TRANSACTION RESULTS:`);
    console.log(`💰 SOL Balance: ${(newSolBalance / LAMPORTS_PER_SOL).toFixed(6)} SOL (${solDiff > 0 ? '+' : ''}${(solDiff / LAMPORTS_PER_SOL).toFixed(6)})`);
    console.log(`🪙 Token Balance: ${newTokenBalance.value.amount} (-${tokenDiff.toString()})`);
    console.log(`💵 SOL Received: ${((solDiff + 5000) / LAMPORTS_PER_SOL).toFixed(6)} SOL (after fees)`);
    ...
    if (error.message?.includes('0xbbd') || error.message?.includes('3005')) {
      console.log('🚨 AccountNotEnoughKeys error - instruction structure issue!');
