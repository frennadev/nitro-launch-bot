...
import type { PoolInfo } from "../backend/get-poolInfo";
import { connection } from "../blockchain/common/connection";
import { getCreatorVaultAuthority } from "../backend/creator-authority";
// NOTE: These functions will be implemented inline as they don't exist in the current codebase
...
interface CreateBuyIXParams {
  ...
  global_volume_accumulator: PublicKey;
  user_volume_accumulator: PublicKey;
  // 🚀 NEW: Fee system accounts
  fee_config: PublicKey;
  pump_fees_program: PublicKey;
}
...
interface CreateSellIXParams {
  ...
  poolInfo: any; // Add poolInfo to get coin_creator for correct vault authority derivation
...
const wrapper_program_id = new PublicKey("6HB1VBBS8LrdQiR9MZcXV5VdpKFb7vjTMZuQQEEPioC");
...
// Helper functions for PDA derivations
function pumpswap_globalVolumeAccumulator(): PublicKey {
...
function pumpswap_userVolumeAccumulator(user: PublicKey): PublicKey {
...
async function prepAtaInstruction(payer: Keypair, owner: PublicKey, mint: PublicKey) {
