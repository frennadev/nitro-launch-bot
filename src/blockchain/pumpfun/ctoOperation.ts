...
export const executeCTOOperation = async (
  ...
  try {
    logger.info(
      `[CTO] Starting CTO operation for token ${tokenAddress}, user ${userId}, amount ${totalAmount} SOL, platform: ${detectedPlatform || "auto-detected"}`
    );
    ...
    logger.info(
      `[CTO] Starting mixer: ${totalAmount} SOL to ${destinationAddresses.length} wallets`
    );
    ...
    if (successfulRoutes.length === 0) {
      logger.error(
        `[CTO] Mixer failed completely - no successful routes:`,
        mixerResult
      );
      return {
        success: false,
    ...
    logger.info(
      `[CTO] Mixer completed with ${mixerResult.successCount || successfulRoutes.length}/${mixerResult.totalRoutes || mixerResult.results?.length} successful routes`
    );

    // Log partial success details
    if (mixerResult.successCount < mixerResult.totalRoutes) {
      logger.info(
        `[CTO] Proceeding with partial mixer success: ${successfulRoutes.length} funded wallets available`
      );
    }
    ...
    const actualAmountsFromMixer = successfulRoutes.map(
      (result) => result.route.amount
    );

    logger.info(
      `[CTO] Using mixer's actual amount distribution: ${actualAmountsFromMixer.map((amt) => (amt / 1e9).toFixed(6)).join(", ")} SOL`
    );
    ...
    logger.info(
      `[CTO] Using ${walletsToUse} wallets that were successfully funded by mixer`
    );

    // NEW: Execute buy transactions sequentially with 50ms delays (no confirmation waiting)
    logger.info(
      `[CTO] Starting sequential buy execution with 50ms delays between wallets`
    );
    ...
    for (let index = 0; index < fundedWallets.length; index++) {
      ...
      logger.info(
        `[CTO] Wallet ${index + 1}/${fundedWallets.length}: Original amount ${buyAmountSol.toFixed(6)} SOL, Adjusted for sell reserve: ${adjustedBuyAmountSol.toFixed(6)} SOL (reserving ${sellReserve} SOL for sells)`
      );
      ...
      try {
        ...
        if (result.success) {
          logger.info(
            `[CTO] Buy ${index + 1}/${fundedWallets.length} sent successfully: ${result.signature}`
          );
          ...
          logger.warn(
            `[CTO] Buy ${index + 1}/${fundedWallets.length} failed to send: ${result.error}`
          );
          buyResults.push({
            success: false,
            error: result.error,
        ...
        logger.error(
          `[CTO] Buy ${index + 1}/${fundedWallets.length} error:`,
          error
        );
        buyResults.push({
          success: false,
    ...
    logger.info(
      `[CTO] All ${fundedWallets.length} buy transactions sent sequentially. Starting background confirmation...`
    );
    ...
    confirmationResults.forEach((result, index) => {
      if (result.status === "fulfilled" && result.value.success) {
        confirmedSuccesses++;
        logger.info(
          `[CTO] Background confirmation successful for wallet ${index + 1}`
        );
      } else {
        confirmedFailures++;
        const error =
        ...
        logger.warn(
          `[CTO] Background confirmation failed for wallet ${index + 1}: ${error}`
        );
      }
    });
    ...
    logger.info(
      `[CTO] CTO operation completed: ${successfulBuys} confirmed successful, ${failedBuys} confirmed failed`
    );
    ...
    return {
      ...
      successfulBuys,
      failedBuys,
      mixerSuccessRate: Math.round(
        ((mixerResult.successCount || successfulRoutes.length) /
          (mixerResult.totalRoutes || mixerResult.results?.length || 1)) *
          100
      ),
      detectedPlatform,
    };
...
async function confirmAndRecordTransaction(
  ...
  try {
    ...
    if (confirmation.value.err) {
      ...
      // Record failed transaction using the enhanced function
      try {
        ...
        await recordTransactionWithActualAmounts(
          ...
          false // Don't parse amounts for failed transactions
        );
      } catch (recordError: any) {
        logger.warn(
          `[CTO Background] Failed to record failed transaction:`,
          recordError
        );
      }
    ...
    try {
      ...
      if (actualAmounts.success && actualAmounts.actualSolSpent) {
        ...
        logger.info(
          `[CTO Background] Actual SOL spent from blockchain: ${actualTransactionAmountSol} SOL`
        );
      } else {
        logger.warn(
          `[CTO Background] Failed to parse actual amounts, using input amount: ${actualAmounts.error}`
        );
      }
    } catch (parseError: any) {
      logger.warn(
        `[CTO Background] Error parsing transaction amounts, using input amount: ${parseError.message}`
      );
    }
    ...
    try {
      ...
      logger.info(
        `[CTO Background] Transaction recorded successfully: ${signature}`
      );
    } catch (recordError: any) {
      logger.warn(
        `[CTO Background] Failed to record successful transaction:`,
        recordError
      );
    }
    ...
    try {
      logger.info(
        `[CTO Background] Collecting transaction fee for ${actualTransactionAmountSol} SOL transaction`
      );
      ...
      if (wallet) {
        ...
        if (feeResult.success) {
          logger.info(
            `[CTO Background] Transaction fee collected successfully: ${feeResult.feeAmount} SOL`
          );
        } else {
          logger.warn(
            `[CTO Background] Transaction fee collection failed: ${feeResult.error}`
          );
        }
      } else {
        logger.warn(
          `[CTO Background] Could not find wallet private key for fee collection: ${walletPublicKey}`
        );
      }
    } catch (feeError: any) {
      logger.error(
        `[CTO Background] Error collecting transaction fee:`,
        feeError.message
      );
    }
