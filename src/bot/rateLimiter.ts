...
const DEFAULT_RATE_LIMITS: Record<string, RateLimitConfig> = {
  ...
  token_operations: {
    ...
    maxRequests: 999999, // Effectively unlimited
    message:
      "⚠️ Token operations are rate limited. Please wait 1 minute between launches.",
  },
  ...
  wallet_operations: {
    ...
    maxRequests: 999999, // Effectively unlimited
    message:
      "⚠️ Wallet operations are rate limited. Please wait 5 seconds between operations.",
  },
  ...
  trading_operations: {
    ...
    maxRequests: 999999, // Effectively unlimited
    message:
      "⚠️ Trading operations are rate limited. Please wait 10 seconds between trades.",
  },
  ...
  admin_operations: {
    ...
    maxRequests: 999999, // Effectively unlimited
    message:
      "⚠️ Admin operations are rate limited. Please wait 10 seconds between commands.",
  },
  ...
  message_handling: {
    ...
    maxRequests: 999999, // Effectively unlimited
    message:
      "⚠️ Too many messages. Please wait 10 seconds before sending another.",
  },
  ...
  callback_queries: {
    ...
    maxRequests: 999999, // Effectively unlimited
    message:
      "⚠️ Too many button clicks. Please wait 5 seconds before trying again.",
  },
};
...
export function createRateLimiter(
  ...
  return async (ctx, next) => {
    ...
    if (entry.count >= config.maxRequests) {
      ...
      const message =
        config.message ||
        `⚠️ Rate limit exceeded. Please wait ${remainingTime} seconds.`;
...
export function rateLimitCommands(): MiddlewareFn<Context> {
  return async (ctx, next) => {
    ...
    if (
      ...
      command.startsWith("/markused") ||
      command.startsWith("/removetoken")
    ) {
      rateLimitType = "admin_operations";
    } else if (command.startsWith("/start") || command.startsWith("/menu")) {
      rateLimitType = "general";
    } else if (
      command.startsWith("/directlaunch") ||
      command.startsWith("/reset") ||
      command.startsWith("/forcefix")
    ) {
