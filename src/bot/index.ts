...
import { TokenResponseGenerator } from "./message";
...
const tokenResponseGenerator = new TokenResponseGenerator();
  ...
  try {
    ...
    if (/^[A-Za-z0-9]{32,44}$/.test(text)) {
      try {
        ...
        // Use the advanced TokenResponseGenerator for unified formatting
        const userId = ctx.chat.id.toString();
        const result = await tokenResponseGenerator.generateTokenResponse(text, ctx, userId, "2");
        if (result) {
          ...
          await ctx.reply("❌ Token not found or failed to load.");
        }
        return;
      } catch (e) {
