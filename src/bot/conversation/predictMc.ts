...
export const predictMcConversation = async (
  ...
  const message = `📊 **Market Cap Predictor**

Current SOL Price: **$${currentSolPrice}**

**PumpFun Token Estimates:**
${pumpfunMcResults.map(r => `• ${r.amount} SOL Buy: **$${r.mc}** MC`).join('\n')}

**Bonk.fun Token Estimates:**
${bonkMcResults.map(r => `• ${r.amount} SOL Buy: **$${r.mc}** MC`).join('\n')}

💡 *Estimates based on current SOL price and bonding curve mechanics*`;
  ...
  while (true) {
    ...
    if (data === "custom_mc_amount") {
      ...
      if (amount > 100) {
        await response.reply("⚠️ Maximum buy amount is 100 SOL for estimation.");
        continue;
      }
      ...
      const customMessage = `💰 **Custom Amount Results**
...
💡 *These are estimates based on bonding curve progression*`;
