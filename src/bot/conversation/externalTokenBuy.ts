...
export const buyExternalTokenConversation = async (conversation: Conversation<Context>, ctx: Context) => {
  ...
  if (buyerWallets.length === 0 && !fundingWallet) {
    await sendMessage(tokenInput, "❌ No wallets available for purchase. Please configure at least one buyer wallet or funding wallet.");
    return conversation.halt();
  }
  ...
  if (buyAmount > totalBalance - 0.001) {
    await sendMessage(amountInput, `❌ Insufficient balance for the requested purchase amount.\n\n<b>Requested Amount:</b> ${buyAmount.toFixed(6)} SOL\n<b>Available Balance:</b> ${totalBalance.toFixed(6)} SOL`, { parse_mode: "HTML" });
    return conversation.halt();
  }
  ...
  if (remainingBuyAmount > 0) {
    await sendMessage(amountInput, "❌ Could not distribute the buy amount across available wallets. Purchase cancelled.");
    return conversation.halt();
  }

  // Confirm purchase
  let distributionText = buyDistribution.map(d => `<b>${d.type} Wallet</b>: ${d.amount.toFixed(6)} SOL`).join("\n");
  await sendMessage(amountInput, `🔍 <b>Confirm External Token Purchase</b>\n\n<b>Token Address:</b> <code>${tokenAddress}</code>\n<b>Total Buy Amount:</b> ${buyAmount.toFixed(6)} SOL\n\n<b>Distribution Across Wallets:</b>\n${distributionText}\n\nProceed with purchase?`, {
    parse_mode: "HTML",
  ...
  if (confirmation.callbackQuery?.data === "confirm_external_buy") {
    try {
      ...
      if (successfulBuys > 0) {
        await sendMessage(confirmation, `✅ <b>External Token Purchase Successful!</b>\n\n<b>Successful Purchases:</b> ${successfulBuys} out of ${buyDistribution.length} attempts\n<b>Total Amount Spent:</b> ${totalBought.toFixed(6)} SOL\n<b>Token Address:</b> <code>${tokenAddress}</code>`, { parse_mode: "HTML" });
      } else {
        await sendMessage(confirmation, `❌ <b>External Token Purchase Failed</b>\n\nNo purchases could be completed.`, { parse_mode: "HTML" });
      }

      if (failedBuys.length > 0) {
        await sendMessage(confirmation, `⚠️ <b>Failed Purchases:</b> ${failedBuys.length} attempt(s) failed.`, { parse_mode: "HTML" });
      }
    } catch (error: any) {
